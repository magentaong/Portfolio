...
function DevlogEntry({
  ...
  return (
    <li className={`border-b last:border-b-0 ${NOTEBOOK_RULE}`}>
      <article
        id={`note-${entry.id}`}
      ...
      >
        <header className="flex items-baseline justify-between gap-4 sm:block sm:pr-5">
          ...
          >
            {labels.noteMarker}{entry.id}
          </a>
          <time dateTime={entry.publishedAt} className="font-mono text-[10px] tabular-nums text-[var(--folio-muted)] sm:mt-3 sm:block sm:max-w-[5rem] sm:leading-4">
            {entry.date}
          </time>
        </header>

        <div className={`min-w-0 border-l pl-4 sm:pl-7 ${ink?.rule ?? NOTEBOOK_RULE}`}>
          <dl className="flex flex-wrap gap-x-5 gap-y-1.5 font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--folio-muted)]">
            <div className="flex gap-2"><dt>{labels.project}</dt><dd className="text-[var(--folio-ink)]">{entryProject(entry) || labels.unfiledProject}</dd></div>
            <div className="flex gap-2"><dt>{labels.mood}</dt><dd className="text-[var(--folio-ink)]">{moodLabel}</dd></div>
          </dl>

          <div className={entry.marginNote ? "mt-4 grid min-w-0 gap-7 xl:grid-cols-[minmax(0,1fr)_9.5rem]" : "mt-4 min-w-0"}>
            <div className="min-w-0">
              <h2 id={titleId} className={`max-w-[26ch] text-[clamp(1.65rem,3vw,2.75rem)] font-semibold leading-[1.05] tracking-[-0.035em] ${ink?.text ?? "text-[var(--folio-ink)]"}`}>
                {entry.title}
              </h2>
              <div className="mt-5 max-w-[68ch] space-y-4 text-[15px] leading-7 text-[var(--folio-muted)] md:text-base md:leading-7">
                {paragraphs(entry.body).map((paragraph, index) => <p key={`${entry.id}-paragraph-${index}`} className="whitespace-pre-line">{paragraph}</p>)}
              </div>

              {entry.artifact && <div className={`mt-7 border-y py-3 ${NOTEBOOK_RULE}`}><NotebookImage asset={entry.artifact} sizes="(min-width: 1280px) 560px, (min-width: 1024px) 50vw, 100vw" transcriptionLabel={labels.transcription} captionClassName="mt-2 font-mono text-[10px] leading-4 text-[var(--folio-muted)]" /></div>}

              {entry.code && (
                <figure className={`mt-7 border-y py-4 ${NOTEBOOK_RULE}`}>
                  <figcaption className={`font-mono text-[10px] uppercase tracking-[0.12em] ${NOTEBOOK_BLUE}`}>{entry.code.label}{entry.code.language && <span className="text-[var(--folio-muted)]"> / {entry.code.language}</span>}</figcaption>
                  <pre tabIndex={0} aria-label={entry.code.label} className="mt-3 overflow-x-auto bg-[var(--folio-panel)] p-4 font-mono text-xs leading-6 text-[var(--folio-ink)] focus-visible:outline-[var(--folio-cobalt)]"><code>{entry.code.content}</code></pre>
                </figure>
              )}

              {entry.links && entry.links.length > 0 && (
                <nav aria-label={`${labels.links}: ${entry.title}`} className={`mt-7 border-t pt-4 ${NOTEBOOK_RULE}`}>
                  <p className={`font-mono text-[10px] uppercase tracking-[0.12em] ${NOTEBOOK_BLUE}`}>{labels.links}</p>
                  <ul className="mt-2 flex flex-wrap gap-x-5 gap-y-2 text-sm">
                    {entry.links.map((link) => <li key={`${link.href}-${link.label}`}><a href={link.href} target={link.target === "blank" ? "_blank" : undefined} rel={link.target === "blank" ? "noreferrer" : undefined} className={`underline decoration-[var(--folio-cobalt-rule)] underline-offset-4 transition-colors hover:decoration-current motion-reduce:transition-none ${NOTEBOOK_BLUE}`}>{link.label}{link.target === "blank" && <span aria-hidden="true"> ↗</span>}</a></li>)}
                  </ul>
                </nav>
              )}

              {entry.tags && entry.tags.length > 0 && (
                <div className={`mt-7 flex flex-wrap items-baseline gap-x-4 gap-y-2 border-t pt-4 font-mono text-[10px] uppercase tracking-[0.1em] ${NOTEBOOK_RULE}`}>
                  <p className="text-[var(--folio-muted)]">{labels.tags}</p>
                  <ul aria-label={labels.tags} className="flex flex-wrap gap-x-2 text-[var(--folio-ink)]">
                    {entry.tags.map((tag, index) => <li key={tag}>{tag}{index < entry.tags!.length - 1 && <span aria-hidden="true" className="ml-2 text-[var(--folio-muted)]">/</span>}</li>)}
                  </ul>
                </div>
              )}
            </div>
            ...
            {entry.marginNote && (
              <aside aria-label={labels.marginNote} className="self-start">
                <p className={`mb-2 font-mono text-[9px] uppercase tracking-[0.12em] ${NOTEBOOK_BLUE}`}>{labels.marginNote}</p>
                <NotebookImage asset={entry.marginNote} sizes="(min-width: 1280px) 152px, (min-width: 1024px) 62vw, (min-width: 640px) calc(100vw - 64px), calc(100vw - 40px)" transcriptionLabel={labels.transcription} captionClassName="mt-2 font-mono text-[9px] leading-4 text-[var(--folio-muted)]" />
              </aside>
            )}
          </div>
        </div>

        <DoodleSlot slot="devlog-entry-margin" anchorId={entry.id} className="mt-4 justify-end sm:absolute sm:bottom-8 sm:left-0 sm:mt-0 sm:w-[6.25rem] sm:justify-center" />
      </article>
    </li>
  );
}
