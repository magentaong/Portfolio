      ...
      <header className="absolute inset-x-0 top-0 z-50 text-[#f3efe7]">
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <Link
            ...
            className="flex min-h-11 min-w-0 items-center text-sm font-semibold tracking-tight focus-visible:outline-[#f3efe7]"
          >
            <span className="shrink-0">
              {siteData.brand}<span className="text-[var(--folio-accent)]">.</span>
            </span>
            <span className="ml-3 hidden truncate border-l border-white/25 pl-3 text-[10px] font-normal lowercase text-white/45 lg:inline">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex h-11 w-11 items-center justify-center border border-white/25 text-white/80 transition-colors hover:border-white hover:text-white focus-visible:outline-[#f3efe7]"
              aria-label={
                menuOpen
              ...
              aria-controls="archive-navigation-menu"
            >
          ...
          <div className="absolute inset-x-0 top-full mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12">
            ...
            >
              <div className="flex max-h-[calc(100svh-5rem)] flex-col gap-4 overflow-y-auto overscroll-contain p-5">
                {siteData.archiveNavigation.map((item) => {
                  const active =
                    pathname === item.href ||
                  ...
                  return (
                    <Link
                      ...
                      className={
                        active
                          ? "flex min-h-11 items-center text-lg text-white focus-visible:outline-[#f3efe7]"
                          : "flex min-h-11 items-center text-lg text-white/80 focus-visible:outline-[#f3efe7]"
                      }
                    >
                      {item.label}
                    </Link>
                ...
                <div className="mt-2 flex items-center justify-between border-t border-white/15 pt-4">
                  <Link
                    href={siteData.links.home.href}
                    onClick={() => setMenuOpen(false)}
                    className="inline-flex min-h-11 items-center focus-visible:outline-[#f3efe7]"
                  >
