...
export default function FloatingBalls({ count = 15 }) {
  ...
  const createBalls = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      ball.className =
        "fixed rounded-full pointer-events-auto cursor-move mix-blend-difference filter blur-[1px]";
  ...
  const animateBall = (ball: HTMLDivElement) => {
    ...
    const animate = () => {
      ...
      if (
        x <= 0 ||
        x >= window.innerWidth - Number.parseFloat(ball.style.width)
      ) {
        vx = -vx;
      ...
      if (
        y <= 0 ||
        y >= window.innerHeight - Number.parseFloat(ball.style.height)
      ) {
        vy = -vy;
        // Add a little "bounce" effect
        ball.style.transform = "scale(1.1)";
