export type TimelineEntry = {
  date: string
  title: string
  company: string
  description: string
}

export const timelineData: TimelineEntry[] = [
  {
    date: "May 2026 - Present",
    title: "Backend Software Engineer Intern",
    company: "foodpanda",
    description: "Contributing to backend engineering work involving Go services, API integration, feature flagging, and service migration within a marketplace environment.",
  },
    {
    date: "Sep 2025 - Present",
    title: "Software Engineer (Citizen Developer)",
    company: "People's Association",
    description: "Software Engineer on a part-time basis, working with Mountbatten CIT under Sparks Division to further development of CompostKaki (see projects)",
  },
    {
    date: "Sep 2025 - Dec 2025",
    title: "Software Developer Intern",
    company: "CPF Board",
    description: "Software Developer with a focus on automation, automating day-to-day tasks, and worked on projects which aims to help HR automate repeatable processes like leave approvals etc.",
  },
  {
    date: "Mar 2025 - Present?",
    title: "Freelance Designer and Developer",
    company: "Self-Employed",
    description: "Designing and developing custom websites for clients. Utilizing Next.js, Vercel, and Figma to create responsive and visually appealing web experiences.",
  },
  {
    date: "Aug 2023 - Sep 2025",
    title: "Private Tutor",
    company: "Physics, Chemistry, Math, Programming",
    description: "Students achieved 100% A-grades in H2 Physics/Math, avg. 6.5-grade improvement in O-Level Physics. Simplified programming concepts through hands-on mini projects.",
  },
  {
    date: "Dec 2024 - Mar 2025",
    title: "Design Mentor",
    company: "Steamunity",
    description: "Mentored 4 secondary and 2 polytechnic students in design thinking, guiding prototyping and solution development.",
  },
  {
    date: "Dec 2022 - July 2023",
    title: "Staff",
    company: "L.E. Cafe Confectionery and Pastry",
    description: "Optimized logistics across 3 stores, reducing order processing time by 20%. Resolved 95% of inquiries efficiently.",
  },
]